...
struct DecimalClockFace: View {
    ...
    var body: some View {
        Canvas { context, size in
            ...
            context.stroke(rim, with: .color(Theme.ink), lineWidth: 3)

            for tick in 0..<100 {
                let isMajor = tick % 10 == 0
                let angle = Double(tick) / 100 * 2 * .pi - .pi / 2
                var path = Path()
                path.move(to: point(center, angle, radius - (isMajor ? 11 : 6)))
                path.addLine(to: point(center, angle, radius - 2))
                context.stroke(path, with: .color(Theme.ink), lineWidth: isMajor ? 2 : 0.7)
            }
            ...
            for numeral in 1...10 {
                ...
                context.draw(
                    Text("\(numeral)")
                        .font(.system(size: 15, weight: .semibold, design: .serif))
                        .foregroundStyle(Theme.ink),
                    at: point(center, angle, radius - 24)
                )
            }
            ...
            drawHand(context, center, turns: hourTurns, length: radius * 0.45, width: 4, color: Theme.blue)
            drawHand(context, center, turns: minuteTurns, length: radius * 0.66, width: 2.5, color: Theme.ink)
