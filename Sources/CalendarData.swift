import Foundation

// Static data for the French Republican calendar.
enum RepublicanData {

    static let months = [
        "Vendémiaire", "Brumaire", "Frimaire",
        "Nivôse", "Pluviôse", "Ventôse",
        "Germinal", "Floréal", "Prairial",
        "Messidor", "Thermidor", "Fructidor",
    ]

    // What each month name evokes, shown as a small caption.
    static let monthMeanings = [
        "mois des vendanges", "mois des brumes", "mois des frimas",
        "mois des neiges", "mois des pluies", "mois des vents",
        "mois de la germination", "mois des fleurs", "mois des prairies",
        "mois des moissons", "mois des chaleurs", "mois des fruits",
    ]

    static let monthMeaningsEN = [
        "month of grape harvest", "month of mist", "month of frost",
        "month of snow", "month of rain", "month of wind",
        "month of germination", "month of flowers", "month of meadows",
        "month of harvest", "month of heat", "month of fruit",
    ]

    static let monthTranslations = [
        "Grape Harvest", "Mist", "Frost",
        "Snow", "Rain", "Wind",
        "Germination", "Flower", "Meadow",
        "Harvest", "Heat", "Fruit",
    ]

    // Etymology of each month name, coined by Fabre d'Églantine.
    static let monthEtymologiesFR = [
        "du latin vindemia, la vendange",
        "des brumes et brouillards de l'automne",
        "du frimas, le froid sec de novembre",
        "du latin nivosus, neigeux",
        "du latin pluviosus, pluvieux",
        "du latin ventosus, venteux",
        "du latin germen, la graine qui germe",
        "du latin flos, la fleur",
        "de la prairie qui verdit",
        "du latin messis, la moisson",
        "du grec thermon, la chaleur d'été",
        "du latin fructus, le fruit",
    ]

    static let monthEtymologiesEN = [
        "from Latin vindemia, the grape harvest",
        "from the mists and fogs of autumn",
        "from frimas, the dry cold of November",
        "from Latin nivosus, snowy",
        "from Latin pluviosus, rainy",
        "from Latin ventosus, windy",
        "from Latin germen, the sprouting seed",
        "from Latin flos, flower",
        "from the greening meadows",
        "from Latin messis, the harvest",
        "from Greek thermon, summer heat",
        "from Latin fructus, fruit",
    ]

    // The Times of London mocked the months with these nicknames.
    static let monthNicknames = [
        "Wheezy", "Sneezy", "Freezy",
        "Slippy", "Drippy", "Nippy",
        "Showery", "Flowery", "Bowery",
        "Hoppy", "Croppy", "Poppy",
    ]

    static let seasonsFR = ["Automne", "Hiver", "Printemps", "Été"]
    static let seasonsEN = ["Autumn", "Winter", "Spring", "Summer"]

    // Each weekday name is just a Latin ordinal plus "di" (day).
    static let decadeRoots: [(latin: String, fr: String, en: String)] = [
        ("primus", "premier", "first"),
        ("duo", "deux", "two"),
        ("tres", "trois", "three"),
        ("quattuor", "quatre", "four"),
        ("quinque", "cinq", "five"),
        ("sex", "six", "six"),
        ("septem", "sept", "seven"),
        ("octo", "huit", "eight"),
        ("novem", "neuf", "nine"),
        ("decem", "dix", "ten"),
    ]

    // The ten days of the décade.
    static let decadeDays = [
        "Primidi", "Duodi", "Tridi", "Quartidi", "Quintidi",
        "Sextidi", "Septidi", "Octidi", "Nonidi", "Décadi",
    ]

    static let decadeAbbrev = [
        "Pri", "Duo", "Tri", "Qua", "Qui", "Sex", "Sep", "Oct", "Non", "Déc",
    ]

    // The five (or six, in leap years) complementary days.
    static let complementaryDays = [
        "Fête de la Vertu",
        "Fête du Génie",
        "Fête du Travail",
        "Fête de l'Opinion",
        "Fête des Récompenses",
        "Fête de la Révolution",
    ]

    static let complementaryDaysEN = [
        "Festival of Virtue",
        "Festival of Talent",
        "Festival of Labour",
        "Festival of Convictions",
        "Festival of Honours",
        "Festival of the Revolution",
    ]

    // Every day of the year honors a plant, animal, mineral or tool.
    // 12 months x 30 days, indexed by month * 30 + (day - 1).
    static let ruralDays = [
        // Vendémiaire
        "Raisin", "Safran", "Châtaigne", "Colchique", "Cheval",
        "Balsamine", "Carotte", "Amaranthe", "Panais", "Cuve",
        "Pomme de terre", "Immortelle", "Potiron", "Réséda", "Âne",
        "Belle de nuit", "Citrouille", "Sarrasin", "Tournesol", "Pressoir",
        "Chanvre", "Pêche", "Navet", "Amaryllis", "Bœuf",
        "Aubergine", "Piment", "Tomate", "Orge", "Tonneau",
        // Brumaire
        "Pomme", "Céleri", "Poire", "Betterave", "Oie",
        "Héliotrope", "Figue", "Scorsonère", "Alisier", "Charrue",
        "Salsifis", "Mâcre", "Topinambour", "Endive", "Dindon",
        "Chervis", "Cresson", "Dentelaire", "Grenade", "Herse",
        "Bacchante", "Azerole", "Garance", "Orange", "Faisan",
        "Pistache", "Macjonc", "Coing", "Cormier", "Rouleau",
        // Frimaire
        "Raiponce", "Turneps", "Chicorée", "Nèfle", "Cochon",
        "Mâche", "Chou-fleur", "Miel", "Genièvre", "Pioche",
        "Cire", "Raifort", "Cèdre", "Sapin", "Chevreuil",
        "Ajonc", "Cyprès", "Lierre", "Sabine", "Hoyau",
        "Érable à sucre", "Bruyère", "Roseau", "Oseille", "Grillon",
        "Pignon", "Liège", "Truffe", "Olive", "Pelle",
        // Nivôse
        "Tourbe", "Houille", "Bitume", "Soufre", "Chien",
        "Lave", "Terre végétale", "Fumier", "Salpêtre", "Fléau",
        "Granit", "Argile", "Ardoise", "Grès", "Lapin",
        "Silex", "Marne", "Pierre à chaux", "Marbre", "Van",
        "Pierre à plâtre", "Sel", "Fer", "Cuivre", "Chat",
        "Étain", "Plomb", "Zinc", "Mercure", "Crible",
        // Pluviôse
        "Lauréole", "Mousse", "Fragon", "Perce-neige", "Taureau",
        "Laurier-thym", "Amadouvier", "Mézéréon", "Peuplier", "Coignée",
        "Ellébore", "Brocoli", "Laurier", "Avelinier", "Vache",
        "Buis", "Lichen", "If", "Pulmonaire", "Serpette",
        "Thlaspi", "Thimelé", "Chiendent", "Trainasse", "Lièvre",
        "Guède", "Noisetier", "Cyclamen", "Chélidoine", "Traîneau",
        // Ventôse
        "Tussilage", "Cornouiller", "Violier", "Troène", "Bouc",
        "Asaret", "Alaterne", "Violette", "Marceau", "Bêche",
        "Narcisse", "Orme", "Fumeterre", "Vélar", "Chèvre",
        "Épinard", "Doronic", "Mouron", "Cerfeuil", "Cordeau",
        "Mandragore", "Persil", "Cochléaria", "Pâquerette", "Thon",
        "Pissenlit", "Sylvie", "Capillaire", "Frêne", "Plantoir",
        // Germinal
        "Primevère", "Platane", "Asperge", "Tulipe", "Poule",
        "Bette", "Bouleau", "Jonquille", "Aulne", "Couvoir",
        "Pervenche", "Charme", "Morille", "Hêtre", "Abeille",
        "Laitue", "Mélèze", "Ciguë", "Radis", "Ruche",
        "Gainier", "Romaine", "Marronnier", "Roquette", "Pigeon",
        "Lilas", "Anémone", "Pensée", "Myrtille", "Greffoir",
        // Floréal
        "Rose", "Chêne", "Fougère", "Aubépine", "Rossignol",
        "Ancolie", "Muguet", "Champignon", "Hyacinthe", "Râteau",
        "Rhubarbe", "Sainfoin", "Bâton d'or", "Chamerisier", "Ver à soie",
        "Consoude", "Pimprenelle", "Corbeille d'or", "Arroche", "Sarcloir",
        "Statice", "Fritillaire", "Bourrache", "Valériane", "Carpe",
        "Fusain", "Civette", "Buglosse", "Sénevé", "Houlette",
        // Prairial
        "Luzerne", "Hémérocalle", "Trèfle", "Angélique", "Canard",
        "Mélisse", "Fromental", "Martagon", "Serpolet", "Faux",
        "Fraise", "Bétoine", "Pois", "Acacia", "Caille",
        "Œillet", "Sureau", "Pavot", "Tilleul", "Fourche",
        "Barbeau", "Camomille", "Chèvrefeuille", "Caille-lait", "Tanche",
        "Jasmin", "Verveine", "Thym", "Pivoine", "Chariot",
        // Messidor
        "Seigle", "Avoine", "Oignon", "Véronique", "Mulet",
        "Romarin", "Concombre", "Échalote", "Absinthe", "Faucille",
        "Coriandre", "Artichaut", "Girofle", "Lavande", "Chamois",
        "Tabac", "Groseille", "Gesse", "Cerise", "Parc",
        "Menthe", "Cumin", "Haricot", "Orcanète", "Pintade",
        "Sauge", "Ail", "Vesce", "Blé", "Chalémie",
        // Thermidor
        "Épeautre", "Bouillon blanc", "Melon", "Ivraie", "Bélier",
        "Prêle", "Armoise", "Carthame", "Mûre", "Arrosoir",
        "Panic", "Salicorne", "Abricot", "Basilic", "Brebis",
        "Guimauve", "Lin", "Amande", "Gentiane", "Écluse",
        "Carline", "Câprier", "Lentille", "Aunée", "Loutre",
        "Myrte", "Colza", "Lupin", "Coton", "Moulin",
        // Fructidor
        "Prune", "Millet", "Lycoperdon", "Escourgeon", "Saumon",
        "Tubéreuse", "Sucrion", "Apocyn", "Réglisse", "Échelle",
        "Pastèque", "Fenouil", "Épine vinette", "Noix", "Truite",
        "Citron", "Cardère", "Nerprun", "Tagette", "Hotte",
        "Églantier", "Noisette", "Houblon", "Sorgho", "Écrevisse",
        "Bigarade", "Verge d'or", "Maïs", "Marron", "Panier",
    ]

    // English translations of the rural day names, same indexing.
    static let ruralDaysEN = [
        // Vendémiaire
        "Grape", "Saffron", "Chestnut", "Autumn crocus", "Horse",
        "Impatiens", "Carrot", "Amaranth", "Parsnip", "Vat",
        "Potato", "Strawflower", "Winter squash", "Mignonette", "Donkey",
        "Four o'clock flower", "Pumpkin", "Buckwheat", "Sunflower", "Winepress",
        "Hemp", "Peach", "Turnip", "Amaryllis", "Ox",
        "Eggplant", "Chili pepper", "Tomato", "Barley", "Barrel",
        // Brumaire
        "Apple", "Celery", "Pear", "Beetroot", "Goose",
        "Heliotrope", "Fig", "Black salsify", "Chequer tree", "Plough",
        "Salsify", "Water caltrop", "Jerusalem artichoke", "Endive", "Turkey",
        "Skirret", "Watercress", "Leadwort", "Pomegranate", "Harrow",
        "Baccharis", "Azarole", "Madder", "Orange", "Pheasant",
        "Pistachio", "Tuberous pea", "Quince", "Service tree", "Roller",
        // Frimaire
        "Rampion", "Cattle turnip", "Chicory", "Medlar", "Pig",
        "Lamb's lettuce", "Cauliflower", "Honey", "Juniper", "Pickaxe",
        "Wax", "Horseradish", "Cedar", "Fir", "Roe deer",
        "Gorse", "Cypress", "Ivy", "Savin juniper", "Fork hoe",
        "Sugar maple", "Heather", "Reed", "Sorrel", "Cricket",
        "Pine nut", "Cork", "Truffle", "Olive", "Shovel",
        // Nivôse
        "Peat", "Coal", "Bitumen", "Sulphur", "Dog",
        "Lava", "Topsoil", "Manure", "Saltpetre", "Flail",
        "Granite", "Clay", "Slate", "Sandstone", "Rabbit",
        "Flint", "Marl", "Limestone", "Marble", "Winnowing fan",
        "Gypsum", "Salt", "Iron", "Copper", "Cat",
        "Tin", "Lead", "Zinc", "Mercury", "Sieve",
        // Pluviôse
        "Spurge-laurel", "Moss", "Butcher's broom", "Snowdrop", "Bull",
        "Laurustinus", "Tinder fungus", "Mezereon", "Poplar", "Axe",
        "Hellebore", "Broccoli", "Bay laurel", "Filbert", "Cow",
        "Box tree", "Lichen", "Yew", "Lungwort", "Billhook",
        "Pennycress", "Rose daphne", "Couch grass", "Knotgrass", "Hare",
        "Woad", "Hazel", "Cyclamen", "Celandine", "Sleigh",
        // Ventôse
        "Coltsfoot", "Dogwood", "Stock flower", "Privet", "Billy goat",
        "Wild ginger", "Italian buckthorn", "Violet", "Goat willow", "Spade",
        "Narcissus", "Elm", "Fumitory", "Hedge mustard", "Goat",
        "Spinach", "Leopard's bane", "Pimpernel", "Chervil", "Twine",
        "Mandrake", "Parsley", "Scurvy-grass", "Daisy", "Tuna",
        "Dandelion", "Wood anemone", "Maidenhair fern", "Ash tree", "Dibber",
        // Germinal
        "Primrose", "Plane tree", "Asparagus", "Tulip", "Hen",
        "Chard", "Birch", "Daffodil", "Alder", "Incubator",
        "Periwinkle", "Hornbeam", "Morel", "Beech", "Bee",
        "Lettuce", "Larch", "Hemlock", "Radish", "Beehive",
        "Judas tree", "Romaine lettuce", "Horse chestnut", "Arugula", "Pigeon",
        "Lilac", "Anemone", "Pansy", "Bilberry", "Grafting knife",
        // Floréal
        "Rose", "Oak", "Fern", "Hawthorn", "Nightingale",
        "Columbine", "Lily of the valley", "Mushroom", "Hyacinth", "Rake",
        "Rhubarb", "Sainfoin", "Wallflower", "Fan palm", "Silkworm",
        "Comfrey", "Salad burnet", "Basket of gold", "Orache", "Weeding hoe",
        "Sea thrift", "Fritillary", "Borage", "Valerian", "Carp",
        "Spindle tree", "Chives", "Bugloss", "White mustard", "Shepherd's crook",
        // Prairial
        "Lucerne", "Daylily", "Clover", "Angelica", "Duck",
        "Lemon balm", "Oat grass", "Martagon lily", "Wild thyme", "Scythe",
        "Strawberry", "Betony", "Pea", "Acacia", "Quail",
        "Carnation", "Elderberry", "Poppy", "Linden", "Pitchfork",
        "Cornflower", "Camomile", "Honeysuckle", "Bedstraw", "Tench",
        "Jasmine", "Vervain", "Thyme", "Peony", "Handcart",
        // Messidor
        "Rye", "Oat", "Onion", "Speedwell", "Mule",
        "Rosemary", "Cucumber", "Shallot", "Wormwood", "Sickle",
        "Coriander", "Artichoke", "Clove", "Lavender", "Chamois",
        "Tobacco", "Redcurrant", "Vetchling", "Cherry", "Livestock pen",
        "Mint", "Cumin", "Bean", "Alkanet", "Guineafowl",
        "Sage", "Garlic", "Tare", "Wheat", "Shawm",
        // Thermidor
        "Spelt", "Mullein", "Melon", "Ryegrass", "Ram",
        "Horsetail", "Mugwort", "Safflower", "Blackberry", "Watering can",
        "Foxtail millet", "Glasswort", "Apricot", "Basil", "Ewe",
        "Marshmallow", "Flax", "Almond", "Gentian", "Lock",
        "Carline thistle", "Caper", "Lentil", "Inula", "Otter",
        "Myrtle", "Rapeseed", "Lupin", "Cotton", "Mill",
        // Fructidor
        "Plum", "Millet", "Puffball", "Six-row barley", "Salmon",
        "Tuberose", "Winter barley", "Apocynum", "Liquorice", "Ladder",
        "Watermelon", "Fennel", "Barberry", "Walnut", "Trout",
        "Lemon", "Teasel", "Buckthorn", "Mexican marigold", "Harvest basket",
        "Wild rose", "Hazelnut", "Hops", "Sorghum", "Crayfish",
        "Bitter orange", "Goldenrod", "Maize", "Sweet chestnut", "Pack basket",
    ]
}
